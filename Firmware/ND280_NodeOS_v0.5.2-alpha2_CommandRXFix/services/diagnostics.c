...
static uint32_t EEMEM eeprom_watchdog_reset_count;
...
static uint32_t watchdog_reset_count = 0UL;
...
void diagnostics_init(void)
{
    ...
    counters.uart_duplicate_commands = 0UL;
    counters.eeprom_errors = 0UL;
    counters.watchdog_resets = 0UL;
    millisecond_accumulator = 0U;
    uptime_seconds = 0UL;
    ...
    watchdog_reset_count = eeprom_read_dword(&eeprom_watchdog_reset_count);
    if (watchdog_reset_count == 0xFFFFFFFFUL) watchdog_reset_count = 0UL;
#ifdef WDRF
    if (reset_flags & (1 << WDRF)) {
        watchdog_reset_count++;
        eeprom_update_dword(&eeprom_watchdog_reset_count, watchdog_reset_count);
    }
#endif
    counters.watchdog_resets = watchdog_reset_count;
...
uint32_t diagnostics_get_uptime_milliseconds(void)
{
    uint8_t saved_sreg = SREG;
    uint32_t seconds;
    ...
    milliseconds = millisecond_accumulator;
    SREG = saved_sreg;

    /* Wraparound is harmless for short interval comparisons. */
    return (seconds * 1000UL) + (uint32_t)milliseconds;
}
...
void diagnostics_record_uart_duplicate_command(void) { counters.uart_duplicate_commands++; }
...
void diagnostics_print_response(uint8_t node_id)
{
    ...
    uart0_print(",UART_DUP=");
    uart0_print_uint32(counters.uart_duplicate_commands);
    uart0_print(",EEPROM_ERR=");
    uart0_print_uint32(counters.eeprom_errors);
    uart0_print(",WDT_RESETS=");
    uart0_print_uint32(watchdog_reset_count);
    uart0_print(",RESET=");
