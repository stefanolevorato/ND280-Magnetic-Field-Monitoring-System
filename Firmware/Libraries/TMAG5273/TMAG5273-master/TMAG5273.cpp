...
{  
    ...
    if (i2c_dev->requestFrom((int)currentDeviceAddress, read_bytes) == 0) {  
        Serial.println("Error: Request data failed");  
        return -1;  
    }  
      
    unsigned long start_time = micros();  
    while (!i2c_dev->available()) {  
        if ((micros() - start_time) > 100000) { //timeout 100 ms  
            Serial.println("Error: Wait read timeout");  
            return -1;  
        }  
    }   
    ...
    for (int i=9-read_bytes; i<9; i++) {  
        ...
        if (data_buffer[i] < 0) {  
            Serial.println("Error: Read data failed");  
            return -1;  
        }  
    }  
